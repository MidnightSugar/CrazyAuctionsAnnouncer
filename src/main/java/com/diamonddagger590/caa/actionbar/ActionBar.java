...
public class ActionBar {
	...
	public static void sendTitle(Player player, String title, String subtitle, int fadein, int stay, int fadeout) {
		...
		title = title.replace(Main.color(Main.getListHandler().getConfig().getString("PluginPrefix")), "");
		subtitle = subtitle.replace(Main.color(Main.getListHandler().getConfig().getString("PluginPrefix")), "");
	...
	public static void sendActionBar(Player player, String message) {
		...
		player.spigot().sendMessage(ChatMessageType.ACTION_BAR, TextComponent.fromLegacyText(message.replace(Main.color(Main.getListHandler().getConfig().getString("PluginPrefix")), "")));
		/*Class<?> chatSerializer = getNMSClass("IChatBaseComponent").getDeclaredClasses()[0];
		Class<?> chatComponent = getNMSClass("IChatBaseComponent");
